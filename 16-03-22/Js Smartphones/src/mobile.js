...
class AndroidPhone extends SmartPhone{
    ...
    remainingAmount(priceIHave){
        if(priceIHave - this.price < 0 ) return "Customer has no more amount";
...
class IPhone extends SmartPhone{
    ...
    remainingAmount(priceIHave){
        if(priceIHave - this.price <= 0) return "Customer has no more amount";
        if(priceIHave > this.price) return 'Customer has remaining Rs.'+this.price +' after buying the Iphone';
        if(priceIHave < this.price) return "Customer can't able to buy a phone due to insufficient amount";
    }
}
