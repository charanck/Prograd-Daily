...
function editIssueComponent(issue) {
    document.getElementById("app").innerHTML = "";
    document.getElementById("app").appendChild(navBarComponent());
    const container = newElement("div",{class:'container border-bottom"'},"","");
    ...
    if(issue.getPriority() == 'high') highOption.setAttribute('selected','true');
    prioritySelect.appendChild(highOption);
    const mediumOption = newElement('option',{class:'text-warning'},'Medium','medium')
    prioritySelect.appendChild(mediumOption);
    if(issue.getPriority() == 'medium') mediumOption.setAttribute('selected','true');
    const lowOption = newElement('option',{class:'text-success'},'Low','low')
    prioritySelect.appendChild(lowOption);
    if(issue.getPriority() == 'low') lowOption.setAttribute('selected','true');
    ...
    if(state.user == issue.getCreatedBy() || state.user.getRole() == 'lead') buttons.appendChild(saveButton);
    ...
    // Add comment form
    const addCommentContainer = newElement('div',{class:'container mt-5'},"","");
    const addCommentForm = newElement('form',{ class:'form-group'},"","");
    const addCommentLabel =  newElement('label',{'for':"addcomment"},"Add Comment:","");
    addCommentForm.appendChild(addCommentLabel);
    const addCommentInput = newElement('input',{class:'form-control','placeholder':'Add comment ...',id:"addcomment"},"","");
    addCommentForm.appendChild(addCommentInput);
    addCommentContainer.appendChild(addCommentForm);
    const addCommentButton = newElement('button',{class:"btn btn-success"},"Comment","");
    ...
    // Comment section
    const commentContainer = newElement("div",{class:'container mt-5 bg-light p-3'},"","");
    ...
    Comment.getAllComments().forEach(comment =>{
        if(comment.commentTo == issue){
            const toast = newElement('div',{class:'toast ml-5 my-3',role:'alert',"aria-live":'assertive',"aria-atomic":'true'},"","");
            const toastHeader = newElement('div',{'class':'toast-header'},"","");
            const img = newElement('img',{'src':'./img/profile.jpg',class:'rounded','style':'width:2rem'});
            toastHeader.appendChild(img);
            const strong = newElement('strong',{'class':'mr-auto'},comment.getCommentedBy().getUserName(),"");
            toastHeader.appendChild(strong);
            const small = newElement('small',{class:'text-muted'},` ${comment.getCommentedOn().slice(0,10)} @ ${comment.getCommentedOn().slice(11,16)}`,"");
            toastHeader.appendChild(small);
            const deleteButton = newElement('button',{class:'btn btn-danger ml-5 float-right btn-sm'},"Delete","");
            deleteButton.addEventListener('click',(e)=> handleDeleteComment(comment,issue));
            if(issue.getCreatedBy() == state.user || state.user == comment.getCommentedBy()) toastHeader.appendChild(deleteButton);
            toast.appendChild(toastHeader);
            const toastBody = newElement('div',{class:'toast-body'},comment.getComment(),"");
...
function alertComponent(message,status){
    const alert = newElement('div',{class:`alert alert-${status} alert-dismissible fade show`,"role":'alert'},'','');
    alert.textContent = message;
    const button = newElement('button',{class:'close',"data-dismiss":'alert','aria-label':'close'},"","");
    const span = newElement('span',{'aria-hidden':'true'},"","");
    span.innerHTML = '&times;';
    span.addEventListener('click',(e)=> document.querySelector('#app').style.filter = 'none');
...
function navBarComponent(){
    const navBar = newElement('nav',{class:'navbar navbar-expand-lg navbar-light bg-light'},"","");
    const sitename = newElement('a',{class:'navbar-brand '},"Issue Tracker","");
    navBar.appendChild(sitename);
    const navItems = newElement('div',{class:'navbar-nav d-flec justify-content-between w-100'},"","");
    const homeNavItem = newElement('a',{class:'nav-item nav-link active','href':''},"Home","");
    navItems.appendChild(homeNavItem);
    const logoutNavItem = newElement("a",{class:"nav-item nav-link btn btn-danger text-light btn-sm "},"Log out","");
...
function filterComponent(){
    const filter = newElement('div',{class:"form-check container form-switch my-5 px-3"},"","");
    const filterInput = newElement('a',{class:"btn btn-primary btn-sm mx-3 text-light",},"Toggle","");
    filterInput.addEventListener('click',(e)=>{handleToggleTableCardView(e)});
    const filterLabel = newElement('label',{class:'form-check-label'},"Toggle Card/Table View","");
