...
function handleEditSaveIssue(issue) {
    ...
    User.getAllUsers().forEach((user) => {
        if (user.getUserName() == document.getElementById("assignedto").value)
            assignedTo = user;
    });
...
function handleAddIssue(e) {
    ...
    let assignedTo;
    User.getAllUsers().forEach((user) => {
        if (document.getElementById("assignedto").value == user.getUserName())
            assignedTo = user;
    });
...
function handleRegister() {
    const userName = document.getElementById("username").value;
    if (userName == "")
        return alertComponent("Username shouldn't be empty!!!", "danger");
    let duplicate = false;
    User.getAllUsers().forEach((user) => {
        if (user.getUserName() == userName) duplicate = true;
    });
    if (duplicate)
        return alertComponent("Username already exists!!!", "warning");
    const passWord = document.getElementById("password").value;
    const repeatPassWord = document.getElementById("repeatpassword").value;

    if (passWord != repeatPassWord)
        return alertComponent("Password Doesn't match!!!", "warning");
    const passWordRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,20}$/;
    if (!passWord.match(passWordRegex))
        return alertComponent("Password doesnt match the criteria!!", "danger");
